    ...
    if(type == "Edit"){
     ...
     await addCourse(body).then((res: any) => {
        if (!res?.error) {
          ...
          setAllLessons({
            ...allLessons,
            results: [newCourse, ...(allLessons?.results || [])]
          });
          setOpen(false);
          toast({
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="space-y-4 max-h-[calc(100vh_-_226px)] overflow-auto px-2">
            ...
            <div className="flex justify-end gap-4">
              <Button type="button" className="w-full sm:w-auto" onClick={()=>{setEditData({}),setOpen(false)}}>Cancel</Button>
              <Button type="submit" className="w-full sm:w-auto">
                {type ? titleCase(type) : "Add"} Lesson
              </Button>
            </div>
        </form>
      </Form>
